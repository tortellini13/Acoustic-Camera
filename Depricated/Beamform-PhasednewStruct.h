...
public:
    ...
    void setup(const array3D<float>& data_input);

    void processData(const array3D<float>& data_input, cv::Mat& data_output, int lower_frequency, int upper_frequency, uint8_t post_process_type);
...
private:
    ...
    // Creates FFT plan
    void setupFFT(const array3D<float>& data_input);

    // Performs FFT on audio data
    void FFT(const array3D<float>& data_input);
    ...
    cv::Mat float2DToMat(const array2D<float>& data);
    ...
    array5D<complex<float>> directivity_factor;
    array3D<complex<float>> data_fft;
    array3D<complex<float>> data_beamform;
    //cfloat2D data_fft_collapse;
    array2D<float> data_fft_collapse;
    array2D<float>  data_post_process;
...
void beamform::setupFFT(const array3D<float>& data_input)
{
...
void beamform::setup(const array3D<float>& data_input)
{
    directivity();
    setupFFT(data_input);
} // end setup
...
void beamform::FFT(const array3D<float>& data_input)
{
    // Execute the FFT
...
cv::Mat beamform::float2DToMat(const array2D<float>& data) 
{
...
void beamform::processData(const array3D<float>& data_input, cv::Mat& data_output, int lower_frequency, int upper_frequency, uint8_t post_process_type)
{
